...
import { addProductToCart,removeProductToCart } from '~/services/apiService';
...
export default function useEcomerce() {
    ...
    return {
        ...
        addItem: (newItem, items, group) => {
            ...
            if (itemIndex >= 0) {
                if (group === 'cart') {
                    let body = { product_id: items[itemIndex].id, quantity: items[itemIndex].quantity + newItem.quantity };
                ...
                let body = { product_id: newItem.id, quantity: newItem.quantity };
                addProductToCart(body);
            }
        ...
        removeItem: (selectedItem, items, group) => {
            ...
            let body = { cart_id: selectedItem.cart_id_PK };
                console.log(body);
                removeProductToCart(body);
        },
    };
}
