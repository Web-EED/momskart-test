import React from 'react';

const VendorMileStone = () => (
    <div className="ps-section--vendor ps-vendor-milestone">
        <div className="container">
            <div className="ps-section__header">
                <p>How to Register</p>
                <h4>
                Register in 3 easy steps and start your selling journey


                </h4>
            </div>
            <div className="ps-section__content">
                <div className="ps-block--vendor-milestone">
                    <div className="ps-block__left">
                        <h4>Download and Register</h4>
                        
                         <p>  Download the Momskart Sellers app from Google Play Store and register yourself.</p> 

                           
                    </div>
                    <div className="ps-block__right">
                        <img
                            src="/static/img/vendor/milestone-1.png"
                            alt="martfury"
                        />
                    </div>
                    <div className="ps-block__number">
                        <span>1</span>
                    </div>
                </div>
                <div className="ps-block--vendor-milestone reverse">
                    <div className="ps-block__left">
                        <h4>Complete Profile and List products
</h4>
                        
                         <p>  Complete your profile, list your unique products, and start selling.</p> 

                          
                    </div>
                    <div className="ps-block__right">
                        <img
                            src="/static/img/vendor/milestone-2.png"
                            alt="martfury"
                        />
                    </div>
                    <div className="ps-block__number">
                        <span>2</span>
                    </div>
                </div>
                <div className="ps-block--vendor-milestone">
                    <div className="ps-block__left">
                        <h4>Go live and Start earning</h4>
                        
                       <p> Get verified, go live, and start selling your products.</p>    

                           
                    </div>
                    <div className="ps-block__right">
                        <img
                            src="/static/img/vendor/milestone-3.png"
                            alt="martfury"
                        />
                    </div>
                    <div className="ps-block__number">
                        <span>3</span>
                    </div>
                </div>
                {/* <div className="ps-block--vendor-milestone reverse">
                    <div className="ps-block__left">
                        <h4>Package and ship with ease</h4>
                        <ul>
                            <li>
                                Register your business for free and create a
                                product catalogue. Get free training on how to
                                run your online business
                            </li>
                            <li>
                                Our Momskart Advisors will help you at every
                                step and fully assist you in taking your
                                business online
                            </li>
                        </ul>
                    </div>
                    <div className="ps-block__right">
                        <img
                            src="/static/img/vendor/milestone-4.png"
                            alt="martfury"
                        />
                    </div>
                    <div className="ps-block__number">
                        <span>4</span>
                    </div>
                </div> */}
            </div>
        </div>
    </div>
);

export default VendorMileStone;
